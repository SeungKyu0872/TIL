...
while True:
    ...
    # 0일떄 입력을 그만받자. ==> 돈을 넣어달라고 더이상 말하지 않는다.
    if money == 0:
        break
    ...
    # 1-2. 금액 누적
    # 금액을 넣을때마다 누적 금액에 합산한다. 이후 처음으로 돌아간다.
    budget += money
    # budget = budget + money
...
if budget < 500 :
    ...
    while True:
        print()
    #3-1.구매할 메뉴 선택
        choice = int(input("구매하실 메뉴 번호를 입력하세요 :  "))

    #구매 가능한 메뉴를 골랐다면 구매를 진행
    #구매 가능한 메뉴의 조건 : 번호가 1이상 6이라 and 내가 지금까지 넣은 돈 >= 해당 메뉴의 가격 
     
        if 1 <= choice <= 6 and costs[choice-1] <= budget : 
            break
    
    #구매불가한 메뉴를 골랐다면 구매할수 없다고 출력한 뒤에 3-1로 돌아가야된다.
        print("구매할수 없는 메뉴입니다.")
